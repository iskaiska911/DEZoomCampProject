...
def transform_parquet():
        ...
        spark._jsc.hadoopConfiguration() \
                .set("google.cloud.auth.service.account.json.keyfile",
                     "C://Users/almakarenkov/AppData/Roaming/gcloud/application_default_credentials.json")

        print(spark.version)
        print(len(files))

        for file in files:
                path = f"gs://prefect-de-zoomcamp-alex/{file}.csv.gz.parquet"

                save_path = f"gs://prefect-de-zoomcamp-alex/earthquakes_spark/{file}_parts.csv.gz.parquet"
